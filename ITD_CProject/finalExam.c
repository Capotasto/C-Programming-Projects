...
//  ITD_CProject
//
//  Created by Norio Egi on 4/22/16.
...
void inputNumberToArray(int *array, int index){
    ...
    for (int i = 0; i < SIZE_FINAL; i++) {
        ...
        if (10 <= *(array+i) && *(array+i) <= 99) {
            ...
            puts("WrongNumber!!!It must be two digit!");
            i--;
        }
    }
    puts("");
...
void seachNumber(int *array1, int *array2){
...
void finalExam(){
    ...
    inputNumberToArray(array1, 1);
    inputNumberToArray(array2, 2);
